...
function Perfil_linguagens({ directory, string }: PerfilProps) { // Usamos { } para "pegar" a variável
    return (
        <div className="w-full h-full space-x-2 flex rounded-2xl">
            <div className="w-5 h-5 bg-gray-100 rounded-2xl overflow-hidden">
                <img src={directory}  className="w-full h-full object-cover" />
            </div>
            <span className="text-xs text-gray-600 mt-1">{string}</span>
        </div>
